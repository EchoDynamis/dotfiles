                ...
                chrome.storage.sync
                    ...
                    if ("global" in is_global && is_global["global"]) {
                        if ("exempts" in exempts_list &&
                            ...
                            console.log("GLOBAL MODE IS ON + SITE IS EXEMPTED");
                            chrome.storage.sync
                                .get([message.domain])
                                .then((is_domain) => {
                        ...
                        else if ("override" in is_override &&
                            ...
                            chrome.storage.sync
                                ...
                                .then((is_domain) => {
                                if (message.domain in is_domain) {
                                    console.log("GLOBAL IS ON + OVERRIDE IS OFF + Domain has set fonts");
                                    // @ts-ignore
                                    sendResponse({
                                        type: "apply_font",
                                        data: is_domain[message.domain],
                                    ...
                                    console.log("GLOBAL IS ON + OVERRIDE IS OFF + Domain does not have set fonts");
                                    chrome.storage.sync
                                        .get(["global_fonts"])
                                        .then((result) => {
                                        if ("global_fonts" in result) {
